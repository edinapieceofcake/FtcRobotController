...
public class OpticalOdometry {
    ...
    public Pose2dDual getCurrentPoseDual() {
        ...
        return new Pose2dDual(
                new DualNum(X_SCALAR * pose.x, X_SCALAR * vel.x, X_SCALAR * acc.x),
                new DualNum(Y_SCALAR * pose.y, Y_SCALAR * vel.y, Y_SCALAR * acc.y),
                new DualNum(Math.toRadians(pose.h), Math.toRadians(vel.h), Math.toRadians(acc.h)));
    }
